  ...
  app.post("/chat", async (c) => {
    ...
    return streamSSE(c, async (stream) => {
      ...
      const systemParts: string[] = [
        ...
        "- edit({id?, op, anchor?, text}) mutates the manuscript or a `.md` source. `op:\"replace\"` swaps the first verbatim `anchor` for `text`; `op:\"insert\"` ADDS `text` after the first `anchor` (empty/omitted anchor = top) — it never deletes old text, so do NOT use insert to rewrite; `op:\"overwrite\"` replaces the ENTIRE body with `text` (use for full or section rewrites when an exact anchor is impractical — read all pages first). `id` defaults to the active source; non-markdown sources are read-only via read.",
